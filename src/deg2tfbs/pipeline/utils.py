...
def get_regulator_display_name(regulator: str) -> str:
    """
    Return the correct display name for a transcription factor given its lowercase version.
    If the regulator is not in the mapping, returns the input string.
    """
    display_mapping = {
        'flhd': 'FlhD',
        'flhc': 'FlhC',
    ...
    key = regulator.lower().strip()
    return display_mapping.get(key, regulator)
